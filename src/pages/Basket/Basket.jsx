...
function Basket() {
        ...
        <div className=" m-auto h-96 flex flex-col items-center justify-center">
          ...
          <div className="flex flex-col gap-y-1 items-center justify-center">
            ...
            <p className="text-gray-400">
              Products sahifadagi termalardan boshlang yoki kerakli mahsulotni
              qidiruv orqali toping.
            </p>
            <button
              onClick={() => navigate("/allProducts")}
              className="px-7 p-1 border-2 border-green-600 rounded-lg"
            >
              Products
            </button>
          </div>
  ...
  const contHandle = () => {
    const check = promocodee.find(
      (item) => item.prom.toLowerCase() === promocode.toLowerCase()
    );
    if (check) {
  ...
  const total = basketState.basket
    .map((item) => {
      return item.price;
    })
    .reduce((a, b) => a + b);
        ...
        <div className="flex w-full flex-col gap-y-3 p-7 items-center ">
          <div className="flex w-full items-center justify-between">
            <h1 className="font-bold text-xl">
              Item: {basketState?.basket?.length}
            </h1>
          ...
          <div className="w-full flex flex-col gap-y-2 items-center">
            ...
            <form className="flex gap-y-2 flex-col">
              ...
              <input
                ...
                className="w-full px-4 text-[15px] p-1.5 border rounded outline-none"
          ...
          <div className="flex flex-col gap-y-5 w-full">
            ...
            <button
              onClick={contHandle}
              className="p-1.5 px-5 text-sm bg-green-600 text-white rounded-lg"
            >
              Continue
            </button>
          </div>
